...
def display_pointcloud_if_available(pointcloud_comp, visual, pcd):
...
def freerun():
    ...
    with Harvester() as h:
        ...
        with h.create({'id_': device_id}) as ia:
            ...
            features.SendTexture.value = False
            ...
            # 初始化一个空的点云对象
            # pointcloud = np.random.rand(896000, 3)
            # pcd = o3d.geometry.PointCloud()
            # pcd.points = o3d.utility.Vector3dVector(pointcloud)
            ...
            while True:
                ...
                with ia.fetch(timeout=10) as buffer:
                    ...
                    # print(point_cloud_component)
                    # display_pointcloud_if_available(point_cloud_component, vis, pcd)
                    # time.sleep(1)
                    
                    pointcloud = point_cloud_component.data.reshape(point_cloud_component.height * point_cloud_component.width, 3).copy()
                    print(pointcloud.shape)
                    
                    pointcloud = np.random.rand(896000, 3)  # 生成新的随机点云
                    pcd = o3d.geometry.PointCloud()
                    pcd.points = o3d.utility.Vector3dVector(pointcloud)
